...
def simulate_dynamics_control(Q_total, txt_path, file_fmu):
    ...
    # 模型初始化
    initialize_simplified_chillers(file_fmu_time, file_fmu_state, start_time, stop_time, simulate_time0, Ts,
    ...
    # 控制量变化率du画图
    for i in range(n_input):
        plt.figure(i + 1 + n_output + n_input)
...
def identify_dynamics_simplified_chillers(Q_total_list, txt_path, file_fmu):
    ...
    for i in range(len(Q_total_list)):
        ...
        for j in range(len(object_list)):
            ...
            fmu_address_list = [unzipdir_address, description_address, instance_address]
            write_txt_data(file_fmu_address, fmu_address_list)
            initialize_simplified_chillers(file_fmu_time, file_fmu_state, start_time, stop_time, simulate_time0,
                                           Ts, time_out, tolerance, txt_path)
...
def initialize_simplified_chillers(file_fmu_time, file_fmu_state, start_time, stop_time, time_initialize,
    ...
    # FMU仿真
    result = main_simulate_pause_single(fmu_input_data_default, fmu_input_type, time_initialize, txt_path,
                                        add_input=False)
    # 修改FMU状态
